...
const InfoToast: FC<Props> = ({index, title, position, onPrev, onNext, onDone}) => {
  return (
    <div
    ...
    >
      ...
      <div className='card-footer d-flex justify-content-end p-0 pb-4 pe-8 border-0'>
        <button type='button' onClick={onPrev} className='btn btn-sm btn-light-danger me-2'>
          Prev
          <KTSVG path='/media/icons/arrow-left.svg' className='svg-icon ms-2' />
        </button>
        {onNext && (
          <button type='button' onClick={onNext} className='btn btn-sm btn-light-primary'>
            Next
            <KTSVG path='/media/icons/arrow-right.svg' className='svg-icon ms-2' />
          </button>
        )}
        {onDone && (
          <button type='button' onClick={onDone} className='btn btn-sm btn-primary'>
            OK
          </button>
        )}
      </div>
